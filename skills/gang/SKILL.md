...
### Step 2: Present Results + Auto-Trigger DELIVER
...
4. **Automatically proceed to Stage 6 (DELIVER) — no user prompt needed:**
   - **GO or CONDITIONAL-GO** → print "📦 Generating GO Package..." and immediately run Stage 6 (full deliver path)
   - **NO-GO** → print "📄 Documenting evaluation..." and immediately run Stage 6 (NO-GO documentation path)

   The only exception: if the user has already run DELIVER in this session ("deliver" is already in `stages_completed`) → skip auto-trigger and print "✓ DELIVER already completed for this session."
...
Triggered **automatically** after Stage 5 (ADVISE) completes, OR when the user runs `/gang deliver` manually.
...
3. Route by verdict:
   - **NO-GO** → run **NO-GO Documentation Path** (below), then exit Stage 6
   - **GO or CONDITIONAL-GO** → run **GO Package Path** (below)

---

### PATH A — NO-GO Documentation

Even a NO-GO is valuable work. Document it so the team can revisit later.

**Generate `{output_root}/no-go-summary.md`:**

```markdown
# NO-GO Summary — {evaluation_name}
**Date:** {YYYY-MM-DD} | **Session:** {session_id} | **Mode:** {quality_mode} | **Cost:** ~${cost}

## Verdict: ❌ NO-GO

## Why We Said No
{3-4 sentences from executive-brief.md explaining the core rejection reasons}

## Blocking Risks
{top 3 risks from executive-brief.md that were dealbreakers}

## Failed / Unvalidated Assumptions
| Assumption ID | Text | Status | Impact |
|--------------|------|--------|--------|
{rows from assumptions.json where status = unvalidated or failed, impact = high}

## What Would Need to Change for a Future GO
{kill switches from executive-brief.md reframed as "if these change, re-evaluate"}

## Scores at Time of Rejection
| Dimension | Score | Confidence |
|-----------|------:|:----------:|
{scoring rows from scored-plans.md}
| **Weighted Average** | **{avg}/10** | **{avg_conf}%** |

## Committee Summary
| Agent | Key Finding |
|-------|------------|
{one-line summary per active agent from their position papers}
...
---
*Generated by [Gang v1.3.2](https://github.com/ebnrdwan/GangPlugin)*
```

**Update `{output_root}/state.json`:**
- Add `"deliver"` to `stages_completed`
- Set `current_stage` to `"complete"`

**Print to user:**
```
❌ NO-GO documented.
   Summary: {output_root}/no-go-summary.md

   If conditions change, run /gang reinit to refresh the evaluation.
```

---

### PATH B — GO Package Generation

For **GO** or **CONDITIONAL-GO**: proceed.

### PATH B — Step 1: UX Delta Check (only when UX Researcher was enabled in THINK)
...
### PATH B — Step 2: Dispatch Deliverables Writer
...
### PATH B — After Dispatch

1. Verify all 6 documents exist in `{output_root}/go-package/`
2. Present a summary to the user:
   ```
   ✅ GO Package ready in {output_root}/go-package/
      brd.md · tech-architecture.md · project-charter.md
      risk-register.md · data-model.md · api-contracts.md

   Run /gang push to publish this evaluation to your GitHub Project board.
   ```
